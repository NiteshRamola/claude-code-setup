...
# Hooks
for hook in auto-lint.sh auto-test.sh auto-graph-update.sh guard-destructive.sh guard-sensitive-write.sh secret-guard.sh session-start.sh truncate-test-output.sh; do
  cp "$SETUP_DIR/hooks/$hook" "$HOOKS_DIR/$hook"
  chmod +x "$HOOKS_DIR/$hook"
done
echo "  Installed: 8 hook scripts (executable)"
...
echo "  ~/.claude/hooks/             8 automation scripts"
